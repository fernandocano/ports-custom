...
inherit multilib eutils pax-utils toolchain-enable flag-o-matic
...
src_configure() {
	# protect build system from target flags
	export CFLAGS_FOR_BUILD="-g -fno-stack-protector"
	export CXXFLAGS_FOR_BUILD="-g -fno-stack-protector"
	local confgcc
...
src_compile() {
	...
	if is_crosscompile || tc-is-cross-compiler; then
		...
		replace-flags "-march=corei7" "-mtune=generic"
		emake CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" LIBPATH="${LIBPATH}" bootstrap-lean || die "compile fail"
	fi
}
