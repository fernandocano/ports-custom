...
## STARTX1 ********* first pass patch
#		epatch "${FILESDIR}/cross.patch"
## ENDX1	
# STARTX2
		epatch "${FILESDIR}/cross-p2.patch"
		epatch "${FILESDIR}/cross-p2-extra.patch"
# ENDX2
	fi
...
src_configure() {
	...
	if tc-is-cross-compiler ; then
## STARTX1 **** first pass creates config.sh that is for cross compiling which
##          is done in a second path 
#		myconf "-Dlibc=${SYSROOT}/usr/$(get_libdir)/libc.so"
#		myconf "-Dlibpth=${SYSROOT}/$(get_libdir) ${SYSROOT}/usr/$(get_libdir)"
#
#		cp "${FILESDIR}/config.sh" ${S}/.
#
#		sh Configure \
#		-der -O \
#		-Dusecrosscompile \
...
#		-Dldflags="${LDFLAGS}" \
#		-Dprefix="${SYSROOT}/usr" \
#		-Dsiteprefix="${SYSROOT}/usr" \
#		-Dvendorprefix="${SYSROOT}/usr" \
#		-Dscriptdir="${SYSROOT}/usr/bin" \
#		-Dprivlib="${SYSROOT}${PRIV_LIB}" \
#		-Darchlib="${SYSROOT}${ARCH_LIB}" \
#		-Dsitelib="${SYSROOT}${SITE_LIB}" \
#		-Dsitearch="${SYSROOT}${SITE_ARCH}" \
#		-Dvendorlib="${SYSROOT}${VENDOR_LIB}" \
#		-Dvendorarch="${SYSROOT}${VENDOR_ARCH}" \
#		-Dman1dir=${SYSROOT}/usr/share/man/man1 \
#		-Dman3dir=${SYSROOT}/usr/share/man/man3 \
#		-Dsiteman1dir=${SYSROOT}/usr/share/man/man1 \
#		-Dsiteman3dir=${SYSROOT}/usr/share/man/man3 \
#		-Dvendorman1dir=${SYSROOT}/usr/share/man/man1 \
#		-Dvendorman3dir=${SYSROOT}/usr/share/man/man3 \
#		-Dman1ext='1' \
#		-Dman3ext='3pm' \
#		-Dlibperl="${LIBPERL}" \
#		-Dlocincpth=' ' \
...
#
#		cp config.sh ${FILESDIR}/config-${myarch}.sh
#		die "cross config.sh is done!"
## ENDX1
# STARTX2 ******** Second pass builds miniperl on host
#              and sets up the cross compile Makefile
		sh Configure \
		-des \
		-Darchname="${myarch}" \
		-Dcc="gcc" \
		...
		emake -j1 miniperl || die "emake host miniperl failed"
		emake -j1 uudmap.h || die "emake host miniperl failed"

		cp ${FILESDIR}/config-${myarch}.sh ${S}/Cross/.
		cp ${FILESDIR}/cflags-cross-${myarch} ${S}/.

		export CROSS_NAME=${myarch}
		export CROSSCC=$(tc-getCC)
		CROSS_NAME=${CROSS_NAME} sh Makefile.SH

		mkdir -p ${S}/xlib/${myarch}/CORE
# ENDX2
	else
		sh Configure \
		-des \
...
src_compile() {
	if tc-is-cross-compiler; then
		mf="Makefile-cross-${myarch}"
		emake -j1 -f ${mf} xconfig.h || die "emake xconfig.h failed"
		./miniperl -Ilib configpm --cross=${CROSS_NAME} || die "failed proactive cross config"
		emake -j1 -f ${mf} all || die "emake all failed"
	else
		mf="Makefile"
		# bug 331113
...
src_test() {
	...
	TEST_JOBS=$(echo -j1 ${MAKEOPTS} | sed -r 's/.*(-j[[:space:]]*|--jobs=)([[:digit:]]+).*/\2/' ) \
		make -f ${mf} test_harness || die "test failed"
}
...
#	keepdir "${VENDOR_ARCH}" #338802 for enc2xs
	...
	if use build || tc-is-cross-compiler ; then
		installtarget=install.perl
	fi
	make -f ${mf} DESTDIR="${D}" ${installtarget} || die "Unable to make ${installtarget}"

	rm -f "${D}"/usr/bin/perl
	ln -s perl${MY_PV} "${D}"/usr/bin/perl

	if tc-is-cross-compiler; then
		dolib.a "${D}"/${coredir}/libperl.${myarch}a || die
		dosym libperl.${myarch}a /usr/$(get_libdir)/libperl.a || die
		rm -f "${D}"/${coredir}/libperl.${myarch}a
		dosym ../../../../../$(get_libdir)/libperl.${myarch}a ${coredir}/libperl.${myarch}a
		dosym ../../../../../$(get_libdir)/libperl.${myarch}a ${coredir}/libperl.a
	else
